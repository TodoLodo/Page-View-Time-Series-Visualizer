...
def draw_bar_plot():
    ...
    # Draw bar plot
    fig, ax = plt.subplots()
    df_bar.pivot_table(index='Years', columns=key, values='value', fill_value=0).plot(kind='bar', label='Months',
                                                                                      stacked=False, figsize=(9, 6),
                                                                                      ax=ax)
    my_labels = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October',
                 'November', 'December']
...
def draw_box_plot():
    ...
    # Set tick positions and labels for month-wise box plot
    tick_positions = range(len(months))
